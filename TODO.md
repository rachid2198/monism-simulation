...
- [ ] **1.1** Activate the existing `env/` virtual environment and install JAX (CPU first, GPU later), numpy, h5py. The environment already exists in `F:\Research\Philosophy\monist-simulation\env\`.
- [ ] **1.2** Implement `engine.py`:
...
- [ ] **1.3** Implement `initialize(N, seed)` — uniform S, Normal(0,1) W, hollow matrix, λ_c.
- [ ] **1.4** Write a manual integration test: run N=4 for 100 ticks, print S and active edge count each tick. Verify S sums to 1, W diagonal is 0, sparsity emerges or stabilizes.
- [ ] **1.5** Test with N=100, 1000 ticks. Verify no NaN, no negative S, no self-edges, performance timing.

---

## Phase 2: HDF5 Recording

- [ ] **2.1** Implement `recorder.py`:
...
- [ ] **2.2** Implement adaptive-rate logic: 10 Hz for steps 0–5000, 1 Hz after.
- [ ] **2.3** Write an integration test: run N=4 for 10,000 ticks, verify the HDF5 file has the correct number of snapshots and W_ptr properly indexes sparse edges.
- [ ] **2.4** Write a mini `read_hdf5.py` script that loads a snapshot and prints summary stats. Confirm round-trip correctness.

---

## Phase 3: Simulation Lifecycle & Run Loop

- [ ] **3.1** Implement `lifecycle.py`:
...
- [ ] **3.2** Write an integration test: start, run 2000 ticks, pause, check S sum = 1, resume, run to equilibrium or max_steps.
- [ ] **3.3** Test equilibrium auto-pause: run a non-evolving config (e.g., N=2 with trivial S) and confirm it auto-pauses.
...
- [ ] **4.2** Implement `websocket_handler.py`:
...
- [ ] **4.3** Test with a WebSocket client (e.g., `websocat` or a simple Python script):
...
- [ ] **5.2** Implement `POST /simulation/analyze`:
...
- [ ] **5.3** Test with a paused simulation: call `/analyze`, verify cluster assignments are sensible (at least all nodes assigned, no empty clusters).

---

## Phase 6: Frontend Shell (Vite + React)

- [ ] **6.1** Scaffold Vite + React + TypeScript project:
  - `pnpm create vite frontend --template react-ts` (or npm)
  - Install Three.js, d3-force-3d
- [ ] **6.2** Implement `<ControlPanel>`:
...
- [ ] **6.5** Test: Start simulation via UI, verify status updates, verify WebSocket connects and receives data.

---

## Phase 7: 2D Adjacency Matrix Canvas

- [ ] **7.1** Implement `<AdjacencyMatrix>`:
...
- [ ] **7.2** Implement "Pause & Analyze" re-sort:
  - When analysis results arrive, re-sort the matrix by cluster assignment instead of S
  - Draw cluster boundary lines
- [ ] **7.3** Test with live data: verify the matrix updates, hot corner appears at top-left, re-sort on analysis.

---

## Phase 8: Three.js 3D Force Graph

- [ ] **8.1** Implement Web Worker `forceLayout.worker.ts`:
  - Import d3-force-3d
...
- [ ] **8.4** Test: verify 3D graph renders, nodes size/color reflects S, edges appear, force layout converges.

---

## Phase 9: Integration & Polish

- [ ] **9.1** End-to-end test: Start simulation from UI, watch both views update, pause, analyze, verify cluster re-sort, resume.
...
- [ ] **9.4** GPU setup: Switch from `jax[cpu]` to `jax[cuda12]`, set `XLA_PYTHON_CLIENT_MEM_FRACTION=0.6` before importing JAX to reserve VRAM for the browser's WebGL contexts, test on RTX 3060.
- [ ] **9.5** Seed reproducibility test: Run N=4, seed=42 twice, confirm identical HDF5 outputs.

---

## Phase 10: Documentation & Notebooks

- [ ] **10.1** Write `README.md`: how to install, run, use the UI, interpret results.
- [ ] **10.2** Create `notebooks/analysis_demo.ipynb`: loading HDF5, plotting S over time, plotting active edge count over time, running Louvain, visualizing clusters.
- [ ] **10.3** Add inline code comments and docstrings to all backend functions.
